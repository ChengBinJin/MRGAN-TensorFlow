    ...
    def __call__(self, x):
        ...
        if input_shape[3] == 1:  # unpaired image
            ...
            with tf.variable_scope(self.name + '_unpair', reuse=self.reuse_extra_1):
                ...
                conv7 = tf_utils.conv2d(conv6, 1, k_h=4, k_w=4, d_h=1, d_w=1, padding='SAME',
                                        name='conv7_conv_c1', is_print=True)

                if self.use_sigmoid:
                    output = tf_utils.sigmoid(conv7, name='output_sigmoid_c1', is_print=True)
                else:
                    output = tf_utils.identity(conv7, name='output_without_sigmoid_c1', is_print=True)
            ...
            with tf.variable_scope(self.name + '_pair', reuse=self.reuse_extra_2):
                ...
                # (N, H/16, W/16, 512) -> (N, H/16, W/16, 1)
                conv7 = tf_utils.conv2d(conv6, 1, k_h=4, k_w=4, d_h=1, d_w=1, padding='SAME',
                                        name='conv7_conv_c2', is_print=True)

                if self.use_sigmoid:
                    output = tf_utils.sigmoid(conv7, name='output_sigmoid_c2', is_print=True)
                else:
                    output = tf_utils.identity(conv7, name='output_without_sigmoid_c2', is_print=True)
