    ...
    def __call__(self, x):
        ...
        if input_shape[3] == 1:  # unpaired image
            ...
            with tf.variable_scope(self.name + '_unpair', reuse=self.reuse_extra_1):
                ...
                conv6 = tf_utils.conv2d(conv5, 1, k_h=4, k_w=4, d_h=1, d_w=1, padding='SAME',
                                        name='conv6_conv_c1', is_print=True)

                if self.use_sigmoid:
                    output = tf_utils.sigmoid(conv6, name='output_sigmoid_c1', is_print=True)
                else:
                    output = tf_utils.identity(conv6, name='output_without_sigmoid_c1', is_print=True)
            ...
            with tf.variable_scope(self.name + '_pair', reuse=self.reuse_extra_2):
                ...
                # (N, H/16, W/8, 512) -> (N, H/16, W/16, 1)
                conv6 = tf_utils.conv2d(conv5, 1, k_h=4, k_w=4, d_h=1, d_w=1, padding='SAME',
                                        name='conv6_conv_c2', is_print=True)

                if self.use_sigmoid:
                    output = tf_utils.sigmoid(conv6, name='output_sigmoid_c2', is_print=True)
                else:
                    output = tf_utils.identity(conv6, name='output_without_sigmoid_c2', is_print=True)
