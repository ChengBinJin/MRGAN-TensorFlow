...
class MRIGAN03_LSGAN(object):
    ...
    def discriminator_loss_pair(self, dis_obj, real_img, fake_img):
        ...
        d_loss_real = tf.reduce_mean(tf.squared_difference(d_logit_real, self.real_label))
        d_loss_fake = tf.reduce_mean(tf.square(d_logit_fake))
    ...
    def __call__(self, x):
        ...
        if input_shape[3] == 1:  # unpaired image
            with tf.variable_scope(self.name+'_unpair', reuse=self.reuse_extra_1):
                # Extra-head network
                # (N, H, W, 1) -> (N, H, W, 64)
                conv01 = tf_utils.conv2d(x, self.ndf, k_h=4, k_w=4, d_h=1, d_w=1, padding='SAME', name='conv01_conv_c1')
                conv01 = tf_utils.lrelu(conv01, name='conv01_lrelu_c1', is_print=True)

                conv02 = tf_utils.conv2d(conv01, self.ndf, k_h=4, k_w=4, d_h=1, d_w=1, padding='SAME',
                                         name='conv02_conv_c1')
            ...
            # Share network
            conv4 = self.share_net(conv02, name=self.name)

            with tf.variable_scope(self.name + '_unpair', reuse=self.reuse_extra_1):
                # Extra-tail network
                # (N, H/16, W/16, 512) -> (N, H/16, W/16, 512)
                conv5 = tf_utils.conv2d(conv4, 8 * self.ndf, k_h=4, k_w=4, d_h=1, d_w=1, padding='SAME',
                                        name='conv5_conv_c1')
                conv5 = tf_utils.norm(conv5, _type='instance', _ops=self._ops, name='conv5_norm_c1')
                conv5 = tf_utils.lrelu(conv5, name='conv5_lrelu_c1', is_print=True)

                # (N, H/16, W/16, 512) -> (N, H/16, W/16, 512)
                conv6 = tf_utils.conv2d(conv5, 8 * self.ndf, k_h=4, k_w=4, d_h=1, d_w=1, padding='SAME',
                                        name='conv6_conv_c1')
                conv6 = tf_utils.norm(conv6, _type='instance', _ops=self._ops, name='conv6_norm_c1')
                conv6 = tf_utils.lrelu(conv6, name='conv6_lrelu_c1', is_print=True)

                # (N, H/16, W/16, 512) -> (N, H/16, W/16, 512)
                conv7 = tf_utils.conv2d(conv6, 8 * self.ndf, k_h=4, k_w=4, d_h=1, d_w=1, padding='SAME',
                                        name='conv7_conv_c1')
                conv7 = tf_utils.norm(conv7, _type='instance', _ops=self._ops, name='conv7_norm_c1')
                conv7 = tf_utils.lrelu(conv7, name='conv7_lrelu_c1', is_print=True)

                # (N, H/16, W/16, 512) -> (N, H/16, W/16, 1)
                conv8 = tf_utils.conv2d(conv7, 1, k_h=4, k_w=4, d_h=1, d_w=1, padding='SAME',
                                        name='conv8_conv_c1', is_print=True)

                if self.use_sigmoid:
                    output = tf_utils.sigmoid(conv8, name='output_sigmoid_c1', is_print=True)
                else:
                    output = tf_utils.identity(conv8, name='output_without_sigmoid_c1', is_print=True)
            ...
            with tf.variable_scope(self.name+'_pair', reuse=self.reuse_extra_2):
                # Extra-head network
                # (N, H, W, 1) -> (N, H, W, 64)
                conv01 = tf_utils.conv2d(x, self.ndf, k_h=4, k_w=4, d_h=1, d_w=1, padding='SAME', name='conv01_conv_c2')
            ...
            with tf.variable_scope(self.name + '_pair', reuse=self.reuse_extra_2):
                ...
                conv5 = tf_utils.conv2d(conv4, 8 * self.ndf, k_h=4, k_w=4, d_h=1, d_w=1, padding='SAME',
                                        name='conv5_conv_c2')
                conv5 = tf_utils.norm(conv5, _type='instance', _ops=self._ops, name='conv5_norm_c2')
                conv5 = tf_utils.lrelu(conv5, name='conv5_lrelu_c2', is_print=True)

                # (N, H/16, W/16, 512) -> (N, H/16, W/16, 512)
                conv6 = tf_utils.conv2d(conv5, 8 * self.ndf, k_h=4, k_w=4, d_h=1, d_w=1, padding='SAME',
                                        name='conv6_conv_c2')
                conv6 = tf_utils.norm(conv6, _type='instance', _ops=self._ops, name='conv6_norm_c2')
                conv6 = tf_utils.lrelu(conv6, name='conv6_lrelu_c2', is_print=True)

                # (N, H/16, W/16, 512) -> (N, H/16, W/16, 512)
                conv7 = tf_utils.conv2d(conv6, 8 * self.ndf, k_h=4, k_w=4, d_h=1, d_w=1, padding='SAME',
                                        name='conv7_conv_c2')
                conv7 = tf_utils.norm(conv7, _type='instance', _ops=self._ops, name='conv7_norm_c2')
                conv7 = tf_utils.lrelu(conv7, name='conv7_lrelu_c2', is_print=True)

                # (N, H/16, W/16, 512) -> (N, H/16, W/16, 1)
                conv8 = tf_utils.conv2d(conv7, 1, k_h=4, k_w=4, d_h=1, d_w=1, padding='SAME',
                                        name='conv8_conv_c2', is_print=True)

                if self.use_sigmoid:
                    output = tf_utils.sigmoid(conv8, name='output_sigmoid_c2', is_print=True)
                else:
                    output = tf_utils.identity(conv8, name='output_without_sigmoid_c2', is_print=True)
