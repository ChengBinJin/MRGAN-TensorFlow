...
class CycleGAN(object):
    ...
    def _build_net(self):
        ...
        G_op = self.optimizer(loss=self.G_loss, variables=self.G_gen.variables, name='Adam_G')
        G_ops = [G_op] + self._G_gen_train_ops
        G_optim = tf.group(*G_ops)

        Dy_op = self.optimizer(loss=self.Dy_dis_loss, variables=self.Dy_dis.variables, name='Adam_Dy')
        Dy_ops = [Dy_op] + self._Dy_dis_train_ops
        Dy_optim = tf.group(*Dy_ops)

        F_op = self.optimizer(loss=self.F_loss, variables=self.F_gen.variables, name='Adam_F')
        F_ops = [F_op] + self._F_gen_train_ops
        F_optim = tf.group(*F_ops)

        Dx_op = self.optimizer(loss=self.Dx_dis_loss, variables=self.Dx_dis.variables, name='Adam_Dx')
    ...
    def test_step(self, img, mode='XtoY'):
        if mode == 'XtoY':
            fake_y, recon_x = self.sess.run([self.fake_y_sample, self.recon_x_sample], feed_dict={self.x_test_tfph: img})
            return [img, fake_y, recon_x]
        elif mode == 'YtoX':
            fake_x = self.sess.run(self.fake_x_sample, feed_dict={self.y_test_tfph: img})
            return [img, fake_x, None]
        else:
    ...
    def plots(imgs, iter_time, image_size, save_file):
        ...
        # just save prediction image
        recon_img = imgs[3][0, :, :, 0]
        scipy.misc.imsave(os.path.join(save_file, 're_{}.png').format(str(iter_time).zfill(7)), recon_img)

    @staticmethod
    def plots2(imgs, iter_time, image_size, save_file):
        new_img = np.zeros((image_size[0], 4*image_size[1]))
        imgs = [utils.inverse_transform(imgs[idx]) for idx in range(len(imgs))]
